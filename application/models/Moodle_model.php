...
class Moodle_model extends CI_Model
{
    ...
    {
        parent::__construct();
        $this->db = $this->load->database('db_he', TRUE);
    }
    ...
    function download_user_files($user, $course = NULL, $full = FALSE)
    {        //false= nombre->contenthash
        $query = $this->db
            ->where('userid', $user)
    ...
    function get_user_courses($uid)
    {
        ...
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $enrol) {
                $final[$enrol['id']] = $enrol['courseid'];
            }
            return $final;
        } else {
            return false;
    ...
    function get_user_roles($array, $short = false)
    {
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $rol) {
                if ($short) {
                    $final[$array[$rol['id']]] = $rol['shortname'];
                } else {
                    $final[$array[$rol['id']]] = $rol['roleid'];
                }
            }
            return $final;
        } else {
    ...
    function enrols($cid, $type = NULL)
    {
        ...
        if (!empty($type)) {
            if (is_array($type)) {
                $this->db->where_in('enrol', $type);
            } else {
                $this->db->where('enrol', $type);
            }
        }
    ...
    {
        if (empty($date) or $date > strtotime("now")) {
            $date = "-1 day";
        }
        $date = strtotime($date);
    ...
    {
        if (!empty($level)) {
            $this->db->where('contextlevel', $level);
        }
        if (!empty($instance)) {
    ...
    function course_modules($data, $search = 'course', $manual_order = NULL)
    {
            ...
            foreach (array_values($this->course_modules_order($data)) as $section) {
                if (!empty($section)) {
                    if (is_numeric($section)) {
                        $manual_order[] = (int) $section;
                    } else {
                        array_push($manual_order, $section);
                    }
                }
            }
        ...
        if (!in_array($search, ['course', 'id', 'instance'])) {
            return array();
        }
    ...
    function session_id($uid, $limit = 1)
    {
        ...
        if ($query->num_rows() == $limit) {
            ...
            foreach ($query->result_array() as $sess) {
                $final[] = $sess['sid'];
            }
            return $final;
        } else {
    ...
    function session_by_ip($ip = NULL, $limit = 1, $fullinfo = FALSE)
    {
        ...
        if ($query->num_rows() > 0) {
            if (!$fullinfo) {
                return array_column($query->result_array(), 'sid');
            } else {
                return $query->result_array();
            }
        }
    ...
    function user_assign_grade($uid, $aid, $fulldata = FALSE)
    {
        ...
        if ($query->num_rows() > 0) {
            if ($fulldata) {
                ...
                return array_column($query->result_array(), 'grade', 'assignment');
            }
        } else {
            return array();
        }
    ...
    function user_register($user, $password, $email, $name, $lastname, $data = array(), $force_changepass = true)
    {
        ...
        if ($force_changepass) {
            $this->user_settings($uid, 'auth_forcepasswordchange', '1');
        }
    ...
    function user_settings($user, $config, $value = null)
    {
        ...
        if ($query->num_rows() == 1) {
            if ($value === NULL) {
                ...
                return $this->db->set('value', $value)->where('userid', $user)->where('name', $config)->update('user_preferences');
            }
        } else {
            if ($value === NULL) {
